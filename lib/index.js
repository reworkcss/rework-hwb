...
function convert(string){
  ...
  alpha = params.length == 4;

  if(!(params.length == 3 || alpha)) {
    throw new Error('Wrong number of parameters for hwb ' + params.toString());
  }

  return 'rgb' + (alpha ? 'a(' : '(') + hwb(params[0], params[1], params[2]) + (alpha ? ',' + params[3].trim() + ')' : ')');
}
